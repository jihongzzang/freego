          ...
          <View style={[styles.chartCard, { backgroundColor: colors.surface }]}>
            {Object.keys(state.stats.categoryDistribution).length === 0 ? (
              ...
              <View style={styles.barChart}>
                {Object.entries(state.stats.categoryDistribution).map(
                  ([category, count]) => {
                    const maxCount = Math.max(
                      ...Object.values(state.stats.categoryDistribution)
                    );
          ...
          <View style={[styles.chartCard, { backgroundColor: colors.surface }]}>
            {Object.keys(state.stats.storageDistribution).length === 0 ? (
              ...
              <View style={styles.pieChartContainer}>
                {Object.entries(state.stats.storageDistribution).map(
                  ([location, count]) => {
                    const total = Object.values(
                      state.stats.storageDistribution
                    ).reduce((a, b) => a + b, 0);
                ...
                {state.stats.recentConsumptions.map((item, index) => (
                  <View key={index} style={styles.consumptionItem}>
                    ...
                    <View style={styles.consumptionInfo}>
                      <Text
                        style={[styles.consumptionName, { color: colors.text }]}
                      >
                        {item.ingredient_name}
                      </Text>
                      <Text
                        style={[
                          styles.consumptionDate,
                          { color: colors.textSecondary },
                        ]}
                      >
                        {item.consumed_date}
